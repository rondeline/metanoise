...
function loadFinishedVideo() {
    ...
    submitButton.addEventListener("click", () => {
        ...
        submitResponses(); // Call the function to process responses
    });

    submitButton.classList.add("nav-button", "next"); // Add the "next" class for styling
...
function collectResponses() {
    ...
    trialStimuli.forEach((trial, index) => {
        const selectedRadio = document.querySelector(`input[name="trial-options-${trial.id}"]:checked`);
        if (selectedRadio) {  
            console.log("✅ Trial response:", trial.id, selectedRadio.value);
            responses.push({
                trialId: trial.id,  
                selectedAnswer: selectedRadio.value  
            });
        } else {
            console.log("❌ No response selected for trial", trial.id);
        }
    });
...
function submitResponses() {
    ...
    // Optional: Send to server as well if needed
    fetch('http://stanford-cogsci.org:3959/save_responses.php', {  
        method: 'POST',
...
// ✅ Serve all static files (CSS, JS, images) from the same directory as `run.js`
